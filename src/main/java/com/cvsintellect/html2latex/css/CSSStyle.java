...
package com.cvsintellect.html2latex.css;
...
public class CSSStyle {
	...
	/**
	 * Sets {@link CSSStyle#_start start} and {@link CSSStyle#_end end} commands for the style on the basis of style properties and program configuration.
	 * 
	 * @param conf
	...
	public void makeLaTeXCommands(Configuration conf) {
		...
		for (Iterator<Entry<String, String>> iterator = _properties.entrySet().iterator(); iterator.hasNext();) {
	...
	/*
	 * public void inheritProperties(Configuration conf) { String elementName; if (_name.contains("#")) elementName = _name.substring(0, _name.indexOf('#'));
	 * else if (_name.contains(".")) elementName = _name.substring(0, _name.indexOf('.')); else return;
	 * 
	 * CSSStyle style = conf.getStyle(elementName); if (style == null) return;
	 * 
	 * HashMap<String, String> temp = new HashMap<String, String>(_properties);
	 * 
	 * _properties.putAll(style.getProperties()); _properties.putAll(temp);
	 * 
	 * }
	 */

	/**
	 * Converts &quot;color&quot; property using &quot;xcolor&quot; LaTeX package. HTML notation (#xxx or #xxxxxx where &quot;x&quot; is a hexa number) and rgb
	 * notation (rgb(20,180,60) or rgb(20%, 80%, 15%) are supported. Also the 17 named colours defined defined in the CSS specification are correctly converted.
	 * 
	 * @param conf
	 *            program configuration
	 */
	...
	/**
	 * Converts &quot;font-family&quot; property. Tries to find first generic font family (ie. monospace) used in the definition and converts it using the
	 * configuration.
	 * 
	 * @param conf
	 *            program configuration
	...
	public void fontFamilyProperty(Configuration conf) {
		...
		for (int i = 0; i < fonts.length; ++i) {
			try {
				CSSPropertyConfigItem item = conf.getPropertyConf("font-family" + "-" + fonts[i].trim());
				_start += item.getStart();
				_end = item.getEnd() + _end;
				break;
			}
