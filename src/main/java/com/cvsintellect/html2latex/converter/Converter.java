package com.cvsintellect.html2latex.converter;
...
import com.cvsintellect.html2latex.css.CSSStyle;
import com.cvsintellect.html2latex.exception.FatalErrorException;
import com.cvsintellect.html2latex.exception.NoItemException;
import com.cvsintellect.html2latex.io.IWriter;
import com.cvsintellect.html2latex.io.OutputFileWriter;
import com.cvsintellect.html2latex.io.OutputStringWriter;
import com.cvsintellect.html2latex.main.ProgramInput;
import com.cvsintellect.html2latex.parser.ElementEnd;
import com.cvsintellect.html2latex.parser.ElementStart;

/**
 * Class which converts HTML into LaTeX format. Plain HTML elements are converted using {@link Converter#commonElementStart(ElementStart) commonElementStart()}
 * and {@link Converter#commonElementEnd(ElementEnd, ElementStart) commonElementEnd()} methods. Elements requiring special care during the conversion are
 * converted by calling special methods like {@link Converter#tableRowStart(ElementStart) tableRowStart() }.
 */
public class Converter {
	 ...
	 */
	private HashMap<String, String> _bibliography = new HashMap<String, String>(10);

	public Converter(String configPath) throws FatalErrorException {
		ProgramInput programInput = new ProgramInput();
		programInput.setConfigFile(configPath);
		_config = new Configuration(programInput);

		_writer = new OutputStringWriter();
	}
	
	public Converter(Configuration config) throws FatalErrorException {
		_config = config;
		_writer = new OutputStringWriter();
	}
	 ...
	 */
	public Converter(File outputFile, ProgramInput programInput) throws FatalErrorException {
		_config = new Configuration(programInput);

		_writer = new OutputFileWriter(outputFile);
	}
	...
	private String convertLaTeXSpecialChars(String str) {
		str = str.replace("\\", "@-DOLLAR-\\backslash@-DOLLAR-").replace("&#", "&@-HASH-").replace("$", "\\$").replace("#", "\\#").replace("%", "\\%")
				.replace("~", "\\textasciitilde").replace("_", "\\_").replace("^", "\\textasciicircum").replace("{", "\\{").replace("}", "\\}")
				.replace("@-DOLLAR-", "$").replace("@-HASH-", "#");
	 ...
	 * @param e
	 *            HTML element
	 * @return array with styles in this order: element name style, 'class' style, 'id' style (if style not found null is stored in the array)
	 */
	...
	public void bodyEnd(ElementEnd element, ElementStart es) throws IOException, NoItemException {
		if (!_bibliography.isEmpty()) {
			...
			for (Iterator<Entry<String, String>> iterator = _bibliography.entrySet().iterator(); iterator.hasNext();) {
