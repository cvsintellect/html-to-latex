...
import com.cvsintellect.html2latex.io.IReader;
import com.cvsintellect.html2latex.io.InputFileReader;
import com.cvsintellect.html2latex.io.InputStringReader;
...
public class Parser {
	...
	public Parser(String input, String configPath) throws FatalErrorException {
		_handler = new ParserHandler(configPath);
		_reader = new InputStringReader(input);
	}
	
	public Parser(String input, Configuration config) throws FatalErrorException {
		_handler = new ParserHandler(config);
		_reader = new InputStringReader(input);
	}

	public Parser(File inputFile, IParserHandler handler) throws FatalErrorException {
		_handler = handler;

		_reader = new InputFileReader(inputFile);
	}

	/**
	 * Parses the HTML file and converts it using the particular handler. The file is processed char by char and a couple of events are sent to the handler. The
	 * whole process is very similar to the SAX model used with XML. The list of possible events which are sent to the handler follows.
	 * <ul>
	 ...
	 *            input HTML file
	 * @param handler
	 *            receives events such as startElement (ie. <code>&lt;html)
	 *  &gt;</code>, endElement, ...
	 * @throws FatalErrorException
	 *             fatal error (ie. input file can't be opened) occurs
	 */
	public void parse() throws FatalErrorException {
		try {
			doParsing();
		}
		catch (IOException e) {
			throw new FatalErrorException("Can't read the input file");
		}
		finally {
	...
	/**
	 * Closes the input input file specified in the {@link Parser#parse(File inputFile, IParserHandler handler) parse()} method.
	 * 
	 * @throws FatalErrorException
	 *             when input file can't be closed
	 */
	...
	/**
	 * Reads the input file char by char. When the <code>&quot;&lt;&quot;</code> char is reached {@link Parser#readElement() readElement()} is called otherwise
	...
	/**
	 * Reads elements (tags). Sends <code>comment</code>, <code>startElement</code> and <code>endElement</code> events to the handler.
	 * 
	 * @throws IOException
	 *             when input error occurs
	 */
		...
		while ((c = _reader.read()) != -1) {
			...
			if (ch == '>') {
				...
				if (element instanceof ElementStart) {
					...
					if (str.endsWith("/")) {
						_handler.endElement(new ElementEnd(element.getElementName()), (ElementStart) element);
					}
				}
	...
	/**
	 * Parses element. Stores element attributes in {@link ElementStart ElementStart} object if it's a start element.
	 * 
	 * @param elementString
	 *            string containing the element with its attributes (but without leading &quot;&lt;&quot; and ending &quot;&gt;&quot;)
	 * @return {@link ElementStart ElementStart} or {@link ElementEnd ElementEnd} object.
	 */
	 ...
	 * 
	 * @param firstChar
	 *            first char read in {@link Parser#doParsing doParsing()} method
	 * @throws IOException
	 *             when input error occurs
	 */
	...
	/**
	 * Checks whether the document is well-formed. If not it sends <code>endElement</code> events for the elements which were opened but not correctly closed.
	 * 
	 * @param element
