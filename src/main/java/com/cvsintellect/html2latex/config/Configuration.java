...
package com.cvsintellect.html2latex.config;
...
import com.cvsintellect.html2latex.converter.ElementConfigItem;
import com.cvsintellect.html2latex.converter.LinksConversion;
import com.cvsintellect.html2latex.css.CSSParser;
import com.cvsintellect.html2latex.css.CSSParserHandler;
import com.cvsintellect.html2latex.css.CSSPropertyConfigItem;
...
public class Configuration {
	...
	public Configuration(ProgramInput programInput) throws FatalErrorException {
		loadConfig(programInput);
	}

	protected void loadConfig(ProgramInput programInput) throws FatalErrorException {
		...
		try {
			...
			Document document = builder.parse(new File(programInput.getConfigFile()));
	...
	private void loadElementsConfiguration(Element root) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			_elements.put(elementName, new ElementConfigItem(replaceSpecialStrings(elementStart), replaceSpecialStrings(elementEnd), leaveText, ignoreContent,
					ignoreStyles));
		}
	}

	/**
	 * Loads mapping between HTML named entities (ie. &amp;lt;) and LaTeX commands. Mappings between HTML decimal decimal entities and LaTeX commands are also
	 * loaded.
	 * 
	 * @param root
	 *            root element of the XML configuration file
	...
	private void loadStylesConfiguration(Element root) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			for (int j = 0; j < nl2.getLength(); j++) {
				...
				_stylesConf.put(propertyName + "-" + propertyValue, new CSSPropertyConfigItem(replaceSpecialStrings(start), replaceSpecialStrings(end)));
			}
		}
	}
	...
	private void loadStyleSheet(File f) {
		...
		for (Iterator<Entry<String, CSSStyle>> iterator = _styles.entrySet().iterator(); iterator.hasNext();) {
	...
	private String replaceSpecialStrings(String str) {
		str = str.replace("@NL", "\n").replace("@TAB", "\t").replace("@QUOT", "'").replace("@DOUBLEQUOT", "\"").replace("@AMP", "&").replace("@LT", "<")
				.replace("@GT", ">");
	...
	public String makeCmdsFromCSS() {
		...
		for (Iterator<Entry<String, CSSStyle>> iterator = _styles.entrySet().iterator(); iterator.hasNext();) {
			...
			CSSStyle style = (CSSStyle) entry.getValue();
			ret += "\n\\newcommand{" + getCmdStyleName(styleName) + "}[1]{ " + style.getStart() + "#1" + style.getEnd() + " }";
		}
		return ret + "\n";
	}
	 ...
	 * @param styleName
	 *            style name
	 * @return style name without special chars (ie. &quot;#&quot;) - suitable fo creating new LaTeX command
	 */
